	  ...
	  m_rocket(m_physicsWorld, m_gameLevel)
{
	...
	m_uiPauseTitle.setPosition({static_cast<float>(m_window.getSize().x) / 2.0f,
								(m_uiPauseTitle.getGlobalBounds().height / 2.0f) + 150.0f});
	...
	m_uiResumeButton.setPosition(m_uiPauseTitle.getPosition() +
	...
	m_uiOptionsButton.setPosition(m_uiResumeButton.getPosition() +
	...
	m_uiQuitButton.setPosition(m_uiOptionsButton.getPosition() +
...
void PlayState::updateEndOfLevel(const sf::Time &dt)
{
	(void)dt;
}
